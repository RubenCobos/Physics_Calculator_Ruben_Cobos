// Validates user input by making sure the input requested is of the correct
// type and format. Based on Dr. McMillan's input validation code.
...
/* A polymorphic function the validates user input. Checks if any error flags
 have been set and for extraneous input. If an error is found, the flags are
 reset, the input stream is cleared, and the error is reported. */
...
// Requested input is of type string.

string Validate_Word(string &word) {
  while (true) {
    cout << "Enter an word (no spaces): ";
